	...
	if ( count( $content ) > 1 ) {
		if ( $more ) {
			...
			if ( ! empty( $more_link_text ) )
				 ...
				 */
				$output .= apply_filters( 'the_content_more_link', ' <a href="' . get_permalink() . "#more-{$post->ID}\" class=\"more-link\">Далі</a>", $more_link_text );
...
function get_the_password_form( $post = 0 ) {
	...
	$output = '<h4 class="pass-page-title">Обмежений доступ до сторінки</h4><form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form" method="post">
	<p class="pass-page-text">' . __( 'Щоб отримати доступ, введіть пароль' ) . '</p>
	<p><label for="' . $label . '"><input placeholder="Пароль" name="post_password" id="' . $label . '" type="password" size="20" /></label> <input type="submit" name="Submit" value="Надіслати" /></p></form>
	';
