...
function twentythirteen_setup() {
	...
	register_nav_menu( 'top', __( 'Top Menu', 'twentythirteen' ) );
    register_nav_menu( 'left', __( 'Left Menu', 'twentythirteen' ) );
...
function twentythirteen_widgets_init() {
    ...
    register_sidebar( array(
        'name'          => __( 'Contacts', 'twentythirteen' ),
        'id'            => 'contacts',
        'description'   => __( 'Appears in the footer section of the site.', 'twentythirteen' ),
        'before_widget' => '<aside id="%1$s" class="widget %2$s">',
        'after_widget'  => '</aside>',
...
function twentythirteen_excerpt_more( $more ) {
	$link = sprintf( '<a href="%1$s" class="more-link">%2$s</a>',
		esc_url( get_permalink( get_the_ID() ) ),
			...
			sprintf( __( 'Далі' ), '<span class="screen-reader-text">' . get_the_title( get_the_ID() ) . '</span>' )
		);
	return ' &hellip; ' . $link;
}
...
add_action('init', 'twentythirteen_add_default_boxes');

function twentythirteen_add_default_boxes() {
    register_taxonomy_for_object_type('category', 'graduates');
...
function wpse_category_set_post_types( $query ){
    if( $query->is_category() && $query->is_main_query() ){
        $query->set( 'post_type', array( 'students', 'graduates') );
    }
}
